...
<p align="center"><i>Figure 5.19. Parameter Store contains `/learnsphere/prod/backend-env` as a `SecureString` and `/learnsphere/prod/backend-image-tag` as a `String`. The screenshot proves that runtime configuration and the deployable image tag are managed centrally without exposing decrypted secret values.</i></p>
