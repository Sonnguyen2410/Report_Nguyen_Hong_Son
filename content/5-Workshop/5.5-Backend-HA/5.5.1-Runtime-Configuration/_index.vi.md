...
<p align="center"><i>Hình 5.19. Parameter Store lưu `/learnsphere/prod/backend-env` dưới dạng `SecureString` và `/learnsphere/prod/backend-image-tag` dưới dạng `String`. Ảnh xác nhận cấu hình runtime và image tag triển khai được quản lý tập trung mà không làm lộ giá trị secret đã giải mã.</i></p>
