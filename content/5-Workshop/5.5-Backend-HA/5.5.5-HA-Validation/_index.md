...
<p align="center"><i>Figure 5.25. `LearnSphere-Backend-TG` reports two total targets, two healthy targets, and zero unhealthy targets on HTTP port 5000. Combined with the Multi-AZ placement in Figure 5.24, this confirms that both Backend instances are eligible to receive ALB traffic.</i></p>

![LearnSphere readiness endpoint returns HTTP 200](/images/learnsphere-readiness-200.png)

<p align="center"><i>Figure 5.26. Calling `https://origin.learnspherev2.id.vn/health/ready` through the public ALB returns HTTP 200 with `{"status":"ready","database":"connected"}`, validating the complete TLS, load-balancing, Backend, and database path.</i></p>

![Successful LearnSphere Auto Scaling Group replacement activity](/images/learnsphere-asg-self-healing.png)

<p align="center"><i>Figure 5.27. ASG Activity history records successful instance termination during an Instance Refresh while the group returns to desired capacity with 2/2 healthy instances. This provides operational evidence of controlled replacement and self-healing behavior.</i></p>
