...
<p align="center"><i>Figure 5.20. `LearnSphere-Prod-ALB` is mapped to public subnets in `ap-southeast-1a` and `ap-southeast-1b` inside VPC `10.20.0.0/16`, providing a redundant ingress path across two Availability Zones.</i></p>

![LearnSphere ALB HTTPS listener and Target Group forwarding rule](/images/learnsphere-alb-https-listener.png)

<p align="center"><i>Figure 5.21. The ALB listens on HTTPS port 443, uses the certificate for `origin.learnspherev2.id.vn`, and forwards 100% of matching traffic to `LearnSphere-Backend-TG`.</i></p>
