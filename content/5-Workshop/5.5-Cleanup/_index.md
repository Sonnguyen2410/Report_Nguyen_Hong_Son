...
1. Navigate to **AWS Management Console** $\rightarrow$ Search for and select **CloudFront**.
...
3. **Clean up Frontend Bucket (`learnsphere-fe-575620421319`):**
   - Repeat process: Select `learnsphere-fe-575620421319` $\rightarrow$ Click **Empty** to purge React static assets $\rightarrow$ Click **Delete** to remove the bucket.
...
1. Navigate to **Amazon EC2** $\rightarrow$ Select **Instances**.
2. Select the LearnSphere Backend server (Instance ID: `i-008c48e6c120b2978`).
3. Click **Instance state** menu $\rightarrow$ Select **Terminate instance**.
4. Click **Terminate** to confirm. Status will transition from `Running` $\rightarrow$ `Shutting-down` $\rightarrow$ `Terminated`.

#### Step 4: Delete Amazon ECR Repository

1. Navigate to **Amazon ECR** $\rightarrow$ Select **Private repositories**.
...
1. **Delete CloudWatch Alarms:**
   - Open **CloudWatch** $\rightarrow$ Select **Alarms** $\rightarrow$ **All alarms**.
   - Select the 2 alarms: `LearnSphere-EC2-HighCPU` and `LearnSphere-EC2-StatusCheckFailed`.
   - Click **Actions** $\rightarrow$ Click **Delete**.
2. **Delete CloudWatch Log Group:**
   - Select **Logs** $\rightarrow$ **Log groups**.
   - Find Log Group `/learnsphere/backend`.
   - Click **Actions** $\rightarrow$ Click **Delete log group**.
3. **Delete Amazon SNS Topic & Subscriptions:**
   - Navigate to **Amazon SNS** $\rightarrow$ Select **Topics**.
   - Select Topic `LearnSphere-Alerts` $\rightarrow$ Click **Delete**.
   - Select **Subscriptions** $\rightarrow$ Select associated email subscription $\rightarrow$ Click **Delete**.

#### Step 6: Delete IAM Roles & OIDC Provider

1. **Delete IAM Roles:**
   - Navigate to **IAM** service $\rightarrow$ Select **Roles**.
...
2. **Delete Identity Provider:**
   - In IAM menu $\rightarrow$ Select **Identity providers**.
   - Select Provider `token.actions.githubusercontent.com` $\rightarrow$ Click **Delete**.
