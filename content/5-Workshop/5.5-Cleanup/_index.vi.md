...
1. Truy cập **AWS Management Console** $\rightarrow$ Tìm và chọn dịch vụ **CloudFront**.
...
3. **Dọn dẹp Frontend Bucket (`learnsphere-fe-575620421319`):**
   - Thực hiện tương tự: Chọn bucket `learnsphere-fe-575620421319` $\rightarrow$ Nhấn **Empty** để xóa các tệp tĩnh React $\rightarrow$ Nhấn **Delete** để xóa bucket.

#### Bước 3: Hủy máy chủ ảo Amazon EC2

Hủy máy chủ EC2 sẽ tự động giải phóng địa chỉ IP công cộng và ổ đĩa lưu trữ EBS đính kèm:

1. Truy cập dịch vụ **Amazon EC2** $\rightarrow$ Chọn mục **Instances**.
2. Chọn máy chủ EC2 của ứng dụng LearnSphere (Instance ID: `i-008c48e6c120b2978`).
3. Chọn menu **Instance state** $\rightarrow$ Chọn **Terminate instance**.
4. Nhấn **Terminate** để xác nhận. Trạng thái máy chủ sẽ chuyển từ `Running` $\rightarrow$ `Shutting-down` $\rightarrow$ `Terminated`.

#### Bước 4: Xóa Kho lưu trữ Amazon ECR

1. Truy cập dịch vụ **Amazon ECR** $\rightarrow$ Chọn mục **Private repositories**.
...
1. **Xóa CloudWatch Alarms:**
   - Truy cập **CloudWatch** $\rightarrow$ Chọn **Alarms** $\rightarrow$ **All alarms**.
   - Chọn 2 cảnh báo: `LearnSphere-EC2-HighCPU` và `LearnSphere-EC2-StatusCheckFailed`.
   - Chọn **Actions** $\rightarrow$ Nhấn **Delete**.
2. **Xóa CloudWatch Log Group:**
   - Chọn mục **Logs** $\rightarrow$ **Log groups**.
   - Tìm Log Group `/learnsphere/backend`.
   - Chọn **Actions** $\rightarrow$ Nhấn **Delete log group**.
3. **Xóa Amazon SNS Topic & Subscriptions:**
   - Truy cập dịch vụ **Amazon SNS** $\rightarrow$ Chọn mục **Topics**.
   - Chọn Topic `LearnSphere-Alerts` $\rightarrow$ Nhấn **Delete**.
   - Chọn mục **Subscriptions** $\rightarrow$ Chọn Subscription Email liên quan $\rightarrow$ Nhấn **Delete**.

#### Bước 6: Xóa Phân quyền IAM Roles & OIDC Provider

1. **Xóa IAM Roles:**
   - Truy cập dịch vụ **IAM** $\rightarrow$ Chọn mục **Roles**.
...
2. **Xóa Identity Provider:**
   - Tại menu IAM $\rightarrow$ Chọn mục **Identity providers**.
   - Chọn Provider `token.actions.githubusercontent.com` $\rightarrow$ Nhấn **Delete**.
