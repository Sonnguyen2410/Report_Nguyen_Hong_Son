...
<p align="center"><i>Figure 5.30. CloudFront retains only two production origins: the private S3 bucket for the frontend and the `origin.learnspherev2.id.vn` custom origin backed by the ALB.</i></p>

![CloudFront behaviors route frontend and API traffic to the correct origins](/images/learnsphere-cloudfront-behaviors.png)

<p align="center"><i>Figure 5.31. The `/api/*` behavior routes uncached requests to the ALB, while the Default behavior distributes the frontend from S3.</i></p>

![CloudFront Function source used for SPA route rewriting](/images/learnsphere-cloudfront-spa-function.png)

<p align="center"><i>Figure 5.32. The `LIVE` source of `LearnSphereSpaRewrite2`, which rewrites client-side routes to `index.html`.</i></p>
