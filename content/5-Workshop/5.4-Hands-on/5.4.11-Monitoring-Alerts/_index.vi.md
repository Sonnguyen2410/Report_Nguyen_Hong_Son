...
1. Mở **AWS Management Console** $\rightarrow$ dịch vụ **Amazon SNS** $\rightarrow$ **Topics** $\rightarrow$ chọn **Create topic**.
2. **Type:** `Standard`.
...
1. Mở **CloudWatch Console** $\rightarrow$ **Alarms** $\rightarrow$ **All alarms** $\rightarrow$ chọn **Create alarm**.
2. **Select metric:** Chọn `EC2` $\rightarrow$ `Per-Instance Metrics` $\rightarrow$ chọn metric `CPUUtilization` của EC2 Instance ID `i-008c48e6c120b2978`.
...
1. Chọn **Create alarm** $\rightarrow$ Select metric `StatusCheckFailed` của EC2 Instance ID `i-008c48e6c120b2978`.
...
Mở **CloudWatch** $\rightarrow$ **Alarms** $\rightarrow$ **All alarms**, xác nhận cả 2 Alarm `LearnSphere-EC2-HighCPU` và `LearnSphere-EC2-StatusCheckFailed` đang ở trạng thái **OK**. Hệ thống sẵn sàng tự động phát thông báo qua email bất cứ khi nào hạ tầng xảy ra sự cố.
