...
1. Open **AWS Management Console** $\rightarrow$ **Amazon SNS** service $\rightarrow$ **Topics** $\rightarrow$ click **Create topic**.
2. **Type:** `Standard`.
...
1. Open **CloudWatch Console** $\rightarrow$ **Alarms** $\rightarrow$ **All alarms** $\rightarrow$ click **Create alarm**.
2. **Select metric:** Select `EC2` $\rightarrow$ `Per-Instance Metrics` $\rightarrow$ select `CPUUtilization` for EC2 Instance ID `i-008c48e6c120b2978`.
...
1. Click **Create alarm** $\rightarrow$ Select `StatusCheckFailed` metric for EC2 Instance ID `i-008c48e6c120b2978`.
...
Navigate to **CloudWatch** $\rightarrow$ **Alarms** $\rightarrow$ **All alarms**, confirm both `LearnSphere-EC2-HighCPU` and `LearnSphere-EC2-StatusCheckFailed` display status **OK**. The monitoring infrastructure is ready to dispatch email alerts whenever system anomalies occur.
