...
1. Open **AWS Management Console** $\rightarrow$ navigate to **Amazon CloudFront** $\rightarrow$ click **Create distribution**.
2. **Origin Domain (S3 FE):** Select `learnsphere-fe-575620421319.s3.ap-southeast-1.amazonaws.com`.
3. **Origin Access:** Select **Origin access control settings (recommended)** $\rightarrow$ Create control setting (Sign requests enabled).
...
After Distribution creation, copy the generated OAC Bucket Policy banner and update it in S3 Bucket `learnsphere-fe-575620421319` (**Permissions** tab $\rightarrow$ **Bucket policy**):
...
1. Select **Behaviors** tab $\rightarrow$ click **Create behavior**:
   - **Path pattern:** `/api/*`
